  ...
  useEffect(() => {
    (async () => {
      const [cR, lR] = await Promise.all([
        fetch("/api/contest/get", { cache: "no-store" }),
  ...
  const active = useMemo(
    () =>
      (cfg.contests || [])
        ...
        .sort(
          (a, b) => new Date(a.activeFrom).getTime() - new Date(b.activeFrom).getTime()
        ),
    [cfg, now]
  );
    ...
    <div className="mx-auto max-w-6xl p-6 space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-neutral-300">
          Make memes from today’s prompt image(s). Post on X and share in our Telegram
          memeroom.
        </p>
      </header>
      ...
      <section className="grid md:grid-cols-2 gap-6">
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent className="space-y-4">
            ...
            {active.map((c) => {
              ...
              const imgParam = first
                ? `?img=${encodeURIComponent(first.url)}${
                    c.tweetTemplate
                      ? `&caption=${encodeURIComponent(c.tweetTemplate)}`
                      : ""
                  }`
                : "";
              return (
                <div key={c.id} className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button variant="secondary" asChild>
                      <a
                        href="https://t.me/pareidoliaportal"
                        target="_blank"
                        rel="noreferrer"
                      >
                        Telegram Memeroom
        ...
        <Card className="bg-white/5 border-white/10">
          <CardHeader>
            <CardTitle>Leaderboard (last 14 days)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {lb.length === 0 && (
              <p className="text-neutral-400 text-sm">No ranked entries yet.</p>
            )}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <a
                      ...
                      href={`https://x.com/i/status/${x.id}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      Tweet {x.id}
      ...
      <section className="text-sm text-neutral-400">
        <p>
          Scoring = likes×1 + reposts×2 + replies×2 + quotes×3. Only tweets from the
          last 14 days are counted.
        </p>
      </section>
    </div>
  );
